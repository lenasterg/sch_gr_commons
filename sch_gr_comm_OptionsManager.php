...
class sch_gr_comm_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'sch_gr_commons'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'sch_gr_commons'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'sch_gr_commons'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'sch_gr_commons'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'sch_gr_commons');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'sch_gr_commons'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'sch_gr_commons');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'sch_gr_commons'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'sch_gr_commons') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'sch_gr_commons');
            case 'false':
                return __('false', 'sch_gr_commons');

            case 'Administrator':
                return __('Administrator', 'sch_gr_commons');
            case 'Editor':
                return __('Editor', 'sch_gr_commons');
            case 'Author':
                return __('Author', 'sch_gr_commons');
            case 'Contributor':
                return __('Contributor', 'sch_gr_commons');
            case 'Subscriber':
                return __('Subscriber', 'sch_gr_commons');
            case 'Anyone':
                return __('Anyone', 'sch_gr_commons');
        }
        return $optionValue;
    }
